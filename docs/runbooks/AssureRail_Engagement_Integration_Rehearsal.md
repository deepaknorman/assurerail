...
7. A reviewer with current institution-scoped permission, asset-family qualification and fresh MFA releases or rejects the exact report digest. A sign-off evidence version/digest and qualification snapshot are retained with the decision. The requester cannot release their own report. Changed source evidence or a newer unreleased run prevents an older initial report from authorising preparation.
8. Each stage allows the initial reviewed run plus three included reviewed reruns within 30 days of first release, with eight total processing attempts and one pending run at a time. Changed commercial scope needs a new governed quote; an amendment workflow is not yet supplied.

Current pricing remains ₹500 per unique loan / ₹5L combined minimum for the committed route, ₹650 / ₹6.5L standalone, with 20% of the committed total paid for Initial Assessment and the route-specific remainder before preparation. Execution is additional. The previously discussed ₹8L/₹10.4L minima and ₹3L initial minimum remain proposals. Approved rate-card tax is added explicitly; no universal tax rate is hard-coded.
...
| `ASSURERAIL_QUALIFIED_REVIEWERS_JSON` | Approved reviewer records: `userId`, `assetFamilies`, `expiresAt`, `qualificationRef` |
...
- `POST /:engagementId/runs/:runId/review` requires `RELEASE`/`REJECT`, exact result digest, validated sign-off evidence and MFA. Evidence purpose is `ASSESSMENT_REVIEW:<runId>:<resultDigest>`.
...
Evidence from this increment: API/web builds, 59 targeted Node tests, six synthetic extraction fixtures and a 35-migration disposable PostgreSQL rehearsal. The database test exercises the actual engagement, invoice, checkout, upload, extraction, review and rerun services, while identity/MFA, provider HTTP, clean scanner and reviewer credentials are **synthetic stubs**. The existing invoice/receipt backup-restore checks precede the new engagement scenarios; new engagement records were not separately backup-restored. No real provider charges, AI inference, authenticated browser journey, bank SFTP roundtrip or Castler settlement have been performed.
...
| Full preparation | AssurePool signed seller launch with accepted Rail billing authority, full credit/legal/risk modelling and approved tape mappings; representative document/model accuracy evaluation, customer correction UX and operational reviewer capacity. |
