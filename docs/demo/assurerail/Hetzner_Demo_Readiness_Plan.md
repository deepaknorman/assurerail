...
- Founder supplied a dedicated Rail key locally as `ARAIL_OPENAI_KEY`. Model access returned 200, and the actual Rail adapter returned the expected strict-JSON synthetic marker using OpenAI with no fallback. Hetzner's canonical `ASSURERAIL_OPENAI_API_KEY` and activation flags were still absent at the check; secure mapping/activation is with Claude. Until activated, retain `AI_NOT_RUN` / `AUTOMATED_ANALYSIS_INCOMPLETE`. The local provider check is not a completed hosted assessment.
