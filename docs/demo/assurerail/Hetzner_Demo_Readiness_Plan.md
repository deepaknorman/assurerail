...
- AI credentials/approval remain absent. Demonstrate genuine deterministic parsing/reconciliation while retaining `AI_NOT_RUN` / `AUTOMATED_ANALYSIS_INCOMPLETE` where applicable. This interim result does not satisfy complete AI analysis or expert-reviewed buyer readiness. Do not reuse another product's provider credentials.
