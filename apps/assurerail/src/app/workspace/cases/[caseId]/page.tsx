...
import { availability, customerWorkspaceEnabled, daProductEnabled, evidenceDisplayState, qualificationText, stateTone, type Availability } from "@/lib/customer-workspace";
...
export default function CustomerCasePage() {
  ...
  return <><VenueHeader /><main className="wrap institutional-page customer-workspace">
    ...
    {item && <>
      <div className="institution-title-row"><div className="console-head"><p className="eyebrow">{item.transactionRoute} · {item.representation} · {item.operatingMode}</p><h1>{item.caseReference}</h1><p>{item.assetClass} · {item.lifecycleLeg} · route pack {item.routePackRef}@{item.routePackVersion}</p>{daProductEnabled() && item.transactionRoute === "DA" && item.representation === "CONVENTIONAL" && <Link className="btn btn-primary" href={`/workspace/cases/${encodeURIComponent(caseId)}/da`}>Open DA journey</Link>}</div><span className={`pill ${legallyEffective ? "pill-ok" : "pill-warn"}`}>{item.status}</span></div>
