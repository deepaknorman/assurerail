...
import { availability, customerWorkspaceEnabled, daProductEnabled, evidenceDisplayState, lifecycleProductEnabled, ptcProductEnabled, qualificationText, stateTone, type Availability } from "@/lib/customer-workspace";
...
export default function CustomerCasePage() {
  ...
  return <><VenueHeader /><main className="wrap institutional-page customer-workspace">
    ...
    {item && <>
      <div className="institution-title-row"><div className="console-head"><p className="eyebrow">{item.transactionRoute} · {item.representation} · {item.operatingMode}</p><h1>{item.caseReference}</h1><p>{item.assetClass} · {item.lifecycleLeg} · route pack {item.routePackRef}@{item.routePackVersion}</p><div className="button-row">{daProductEnabled() && item.transactionRoute === "DA" && item.representation === "CONVENTIONAL" && <Link className="btn btn-primary" href={`/workspace/cases/${encodeURIComponent(caseId)}/da`}>Open DA journey</Link>}{ptcProductEnabled() && item.transactionRoute === "PTC" && item.representation === "CONVENTIONAL" && <Link className="btn btn-primary" href={`/workspace/cases/${encodeURIComponent(caseId)}/ptc`}>Open PTC journey</Link>}{lifecycleProductEnabled() && item.status === "COMPLETED" && <Link className="btn" href={`/workspace/cases/${encodeURIComponent(caseId)}/lifecycle`}>Open lifecycle</Link>}</div></div><span className={`pill ${legallyEffective ? "pill-ok" : "pill-warn"}`}>{item.status}</span></div>
