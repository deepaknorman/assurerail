...
type Saga = { id: string; sagaVersion: number; state: string; executionMode: "OBSERVE_ONLY"; legalMechanism: string; considerationCurrency: string; considerationMinorUnits: string; considerationScale: number; historicOutcomeRef: string; planDigest: string; routeEvidenceBundleDigest: string; legs: Leg[] };
...
type Overview = {
  ...
  stages: Stage[];
  externalGates: Array<{ code: string; owner: string; state: string }>;
  ...
  sagas: Saga[];
  breaks: BreakItem[];
  authoritativeRecord: null | { recordType: string; authorityClass: string; recordkeeperInstitutionId: string; declarationEvidenceDigest: string; snapshots: Array<{ id: string; snapshotKind: string; recordReference: string; payloadDigest: string; sourceAsOfAt: string }> };
};

const EMPTY_PLAN = JSON.stringify({
  replayInput: {
    originatorInstitutionId: "", trusteeInstitutionId: "", recordkeeperInstitutionId: "",
    programmeTrust: { programmeOrTrustEvidenceDigest: "", trusteeAppointmentEvidenceDigest: "" },
    poolTransfer: { poolTransferEvidenceDigest: "", poolEligibilityEvidenceDigest: "", poolDigest: "" },
    requiredReviews: {},
    issue: { executedDocumentsEvidenceDigest: "", trancheDefinitionDigest: "", subscriptionEvidenceDigest: "", consideration: { currency: "INR", units: "", scale: 2 }, considerationReference: "", trusteeControlDecisionDigest: "", allotmentEvidenceDigest: "" },
    authoritativeRecord: { recordType: "", recordReference: "", declarationEvidenceDigest: "", beforeDigest: "", afterDigest: "" },
    lifecycleSetup: { requiredNoticeAcknowledgementDigest: "" },
  },
  evidence: {
    PROGRAMME_OR_TRUST: { evidenceObjectId: "" }, TRUSTEE_APPOINTMENT: { evidenceObjectId: "" },
    POOL_TRANSFER: { evidenceObjectId: "" }, POOL_ELIGIBILITY: { evidenceObjectId: "" },
    EXECUTED_DOCUMENTS: { evidenceObjectId: "" }, TRANCHE_DEFINITION: { evidenceObjectId: "" },
    SUBSCRIPTION: { evidenceObjectId: "" }, TRUSTEE_TRANSACTION_CONTROL: { evidenceObjectId: "" },
    ISSUE_OR_ALLOTMENT: { evidenceObjectId: "" }, AUTHORITATIVE_RECORD_DECLARATION: { evidenceObjectId: "" },
    AUTHORITATIVE_RECORD_BEFORE: { evidenceObjectId: "" }, REQUIRED_NOTICE_ACKNOWLEDGEMENT: { evidenceObjectId: "" },
    HISTORIC_PTC_OUTCOME: { evidenceObjectId: "" },
  },
  legalMechanism: "SECURITISATION_TRUST_PTC", historicOutcomeRef: "", historicOutcomeDigest: "",
  authoritativeRecord: { declarationEvidenceRef: "", sourceReferenceId: null, beforeSourceAsOfAt: "" },
  reason: "Historic PTC completion plan assembled from retained transaction evidence",
}, null, 2);
...
export default function ConventionalPtcJourneyPage() {
  ...
  const [repairAuthority, setRepairAuthority] = useState<Record<string, string>>({});
  const keys = useRef<Record<string, string>>({});
  const root = `/v1/rail/cases/${encodeURIComponent(caseId)}/ptc-replay`;
  const enabled = ptcProductEnabled();
  ...
  return <><VenueHeader/><main className="wrap institutional-page customer-workspace">
    ...
    <div className="console-head"><p className="eyebrow">Conventional PTC · {data?.case.operatingMode ?? "shadow"}</p><h1>{data?.case.caseReference ?? "PTC transaction journey"}</h1><p>Programme/trust, pool transfer, reviews, documentation, subscription, trustee control, allotment, authoritative record and lifecycle setup.</p></div>
    <div className="boundary-note">{data?.authorityNotice ?? "This journey is observe-only. It cannot move funds, issue or allot PTCs, send notices, or alter an authoritative register."}</div>
    {!enabled && <div className="msg err">The conventional PTC product journey is disabled.</div>}{error && <div className="msg err" role="alert">{error}</div>}{notice && <div className="msg ok" aria-live="polite">{notice}</div>}
    {data && <>
      <section className="onboarding-stage-grid" aria-label="Conventional PTC journey">{data.stages.map((stage, index) => <article className="onboarding-stage" key={stage.code}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{stage.code.replaceAll("_", " ")}</strong><p>{stage.summary}</p></div><em className="pill">{stage.state}</em></article>)}</section>
      <section className="panel governance-ceremony"><h2 className="section-title">Governed action ceremony</h2><label className="lbl">Authenticator code<input className="field governance-code" inputMode="numeric" autoComplete="one-time-code" value={totp} onChange={(event) => setTotp(event.target.value.replace(/\D/g, "").slice(0, 8))}/></label><p className="meta">Every mutation consumes a purpose-bound step-up. Idempotency keys remain stable across ambiguous retry.</p></section>
      <section className="workspace-grid">
        <article className="panel workspace-module"><h2>External gates</h2><div className="workspace-list">{data.externalGates.map((gate) => <div className="workspace-row" key={gate.code}><span><strong>{gate.code.replaceAll("_", " ")}</strong><small>Owner: {gate.owner}</small></span><span className="pill">{gate.state}</span></div>)}</div></article>
        <article className="panel workspace-module"><h2>Parties & function performers</h2><div className="workspace-list">{data.case.parties.map((party) => <div className="workspace-row" key={`${party.partyRole}:${party.institutionId}`}><span><strong>{party.partyRole}</strong><small>{party.institutionId}</small></span><span className="pill">{party.status}</span></div>)}{data.case.functionAssignments.map((fn) => <div className="workspace-row" key={fn.materialFunction}><span><strong>{fn.materialFunction}</strong><small>{fn.performer} · {fn.performerInstitutionId ?? "no performer"}</small></span><span className="pill">{fn.status}</span></div>)}</div></article>
        <article className="panel workspace-module"><h2>Intake & diligence</h2><p>{data.sources.items.length} owned source reference(s) · {data.evidence.items.length} visible evidence object(s) · {data.rooms.items.length} visible room(s).</p><div className="button-row"><Link className="btn" href={`/institutions/${encodeURIComponent(activeInstitutionId!)}/evidence`}>Evidence register</Link><Link className="btn" href={`/cases/${encodeURIComponent(caseId)}/rooms`}>Diligence rooms</Link></div>{data.evidence.availability === "UNAVAILABLE" && <div className="capability-unavailable"><strong>Evidence unavailable</strong><span>VIEW_EVIDENCE authority is required; unavailable is not zero.</span></div>}</article>
        <article className="panel workspace-module"><h2>Trustee & authoritative record</h2>{data.authoritativeRecord ? <div className="record-card"><strong>{data.authoritativeRecord.recordType}</strong><p className="meta">{data.authoritativeRecord.authorityClass} · recordkeeper {data.authoritativeRecord.recordkeeperInstitutionId}</p>{data.authoritativeRecord.snapshots.map((snapshot) => <p className="meta" key={snapshot.id}>{snapshot.snapshotKind}: {snapshot.recordReference} · {snapshot.payloadDigest}</p>)}</div> : <p>No declaration recorded. Rail is not the legal ownership register.</p>}</article>
        ...
        <article className="panel workspace-module workspace-wide"><h2>Immutable PTC completion plan</h2><p>Use real retained transaction evidence IDs and digests. Add counsel, rating, assurance and servicer blocks only when required for this route. Never use fixtures to close an external gate.</p><textarea className="field mono-json" rows={24} value={plan} onChange={(event) => setPlan(event.target.value)}/><button className="btn btn-primary" disabled={!!busy || totp.length < 6 || data.authorisation?.status !== "APPROVED" || !data.capabilities.canGovernReplay} onClick={() => void createSaga()}>Create observe-only PTC plan</button></article>
        {data.sagas.map((saga) => <article className="panel workspace-module workspace-wide" key={saga.id}><div className="workspace-module-head"><div><h2>PTC completion saga v{saga.sagaVersion}</h2><p>{saga.executionMode} · {saga.state} · {saga.considerationCurrency} {saga.considerationMinorUnits} at scale {saga.considerationScale}</p></div><div className="button-row"><button className="btn" disabled={!!busy} onClick={() => void download(`${root}/sagas/${encodeURIComponent(saga.id)}/comparison`, `assurerail-ptc-${caseId}-comparison.json`)}>Comparison report</button><button className="btn" disabled={!!busy} onClick={() => void download(`${root}/sagas/${encodeURIComponent(saga.id)}/evidence-pack`, `assurerail-ptc-${caseId}-evidence-pack.json`)}>Evidence pack</button></div></div><div className="record-list">{saga.legs.map((leg) => <div className="record-card" key={leg.id}><div className="record-head"><strong>{leg.sequence}. {leg.legType.replaceAll("_", " ")}</strong><span className="pill">{leg.state}</span></div><p className="meta">Owner {leg.participantOwnerInstitutionId} · {leg.performerClass} · expected digest {leg.expectedDigest}</p>{leg.observations.map((obs) => <p className="meta" key={obs.id}>Observation v{obs.version}: {obs.comparisonResult} · {obs.externalReference} · {obs.finalityClass}/{obs.signatureStatus}</p>)}{leg.state === "PLANNED" && leg.participantOwnerInstitutionId === activeInstitutionId && data.capabilities.canOperateRoute && <details className="governance-form"><summary>Record assigned-performer observation</summary><label className="lbl">Observed fact JSON<textarea className="field mono-json" rows={6} value={observationDrafts[leg.id] ?? "{}"} onChange={(event) => setObservationDrafts({ ...observationDrafts, [leg.id]: event.target.value })}/></label><label className="lbl">External reference<input className="field" value={observationRefs[leg.id] ?? ""} onChange={(event) => setObservationRefs({ ...observationRefs, [leg.id]: event.target.value })}/></label><label className="lbl">Evidence object ID<input className="field" value={observationEvidence[leg.id] ?? ""} onChange={(event) => setObservationEvidence({ ...observationEvidence, [leg.id]: event.target.value })}/></label><button className="btn btn-primary" disabled={!!busy || totp.length < 6} onClick={() => void recordObservation(saga, leg)}>Record actual observation</button></details>}{leg.state === "OBSERVED" && leg.participantOwnerInstitutionId === activeInstitutionId && data.capabilities.canOperateRoute && <button className="btn btn-primary" disabled={!!busy || totp.length < 6 || leg.observations.at(-1)?.comparisonResult !== "MATCHED"} onClick={() => void reconcile(saga, leg)}>Independently reconcile matched observation</button>}</div>)}</div></article>)}
        {!!data.breaks.length && <article className="panel workspace-module workspace-wide"><h2>Break repair queue</h2><p>Repairs append corrected evidence; they never rewrite the original observation.</p>{data.breaks.map((item) => <div className="record-card" key={item.id}><div className="record-head"><strong>{item.breakCode}</strong><span className="pill pill-warn">{item.severity} · {item.status}</span></div><p className="meta">Owner {item.ownerInstitutionId} · due {new Date(item.dueAt).toLocaleString("en-IN")}</p>{item.repairActions.map((repair) => <div className="workspace-row" key={repair.id}><span><strong>{repair.actionType}</strong><small>{repair.reason}</small></span><span><span className="pill">{repair.status}</span>{repair.status === "PROPOSED" && repair.proposedByUserId !== venueUser?.id && item.ownerInstitutionId === activeInstitutionId && <span className="button-row"><button className="btn btn-primary" disabled={!!busy || totp.length < 6} onClick={() => void reviewRepair(item, repair, true)}>Approve repair</button><button className="btn btn-danger" disabled={!!busy || totp.length < 6} onClick={() => void reviewRepair(item, repair, false)}>Reject</button></span>}</span></div>)}{item.status === "OPEN" && item.ownerInstitutionId === activeInstitutionId && <details className="governance-form"><summary>Propose corrected observation</summary><label className="lbl">Replacement observed fact JSON<textarea className="field mono-json" rows={8} value={repairDrafts[item.id] ?? "{}"} onChange={(event) => setRepairDrafts({ ...repairDrafts, [item.id]: event.target.value })}/></label><label className="lbl">External reference<input className="field" value={observationRefs[item.id] ?? ""} onChange={(event) => setObservationRefs({ ...observationRefs, [item.id]: event.target.value })}/></label><label className="lbl">Replacement evidence object ID<input className="field" value={repairEvidence[item.id] ?? ""} onChange={(event) => setRepairEvidence({ ...repairEvidence, [item.id]: event.target.value })}/></label><label className="lbl">Repair authority evidence reference<input className="field" value={repairAuthority[item.id] ?? ""} onChange={(event) => setRepairAuthority({ ...repairAuthority, [item.id]: event.target.value })}/></label><button className="btn btn-primary" disabled={!!busy || totp.length < 6} onClick={() => void proposeRepair(item)}>Propose append-only repair</button></details>}</div>)}</article>}
      </section>
    </>}
  </main></>;
}
