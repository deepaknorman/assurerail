...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: "/og-image.png", width: 1200, height: 630, alt: "AssureRail" }],
    },
  };
}
...
export default async function ResourceArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <PublicPage eyebrow="AssureRail field note" title={article.title} lead={article.description} actions={<AssessmentAction label="Assess a portfolio" />}>
