...
test("computePoolManifest is deterministic and order-independent on its input", () => {
  ...
  assert.equal(CoLending.computePoolManifest(a), CoLending.computePoolManifest(reordered));
  ...
  assert.equal(CoLending.computePoolManifest(withUndef), CoLending.computePoolManifest(withNull));
});
...
test("a change to ANY sealed field yields a different manifest (classification, override, disbursed, date)", () => {
  ...
  const m0 = CoLending.computePoolManifest(base);
  assert.notEqual(m0, CoLending.computePoolManifest([{ ...base[0], classificationBucket: "NPA" }]));
  assert.notEqual(m0, CoLending.computePoolManifest([{ ...base[0], verdict: "HARD_EXCLUDE" as const }]));
  assert.notEqual(m0, CoLending.computePoolManifest([{ ...base[0], overridden: true, overrideReason: "in anyway" }]));
  assert.notEqual(m0, CoLending.computePoolManifest([{ ...base[0], disbursedMinor: "1" }]));
  assert.notEqual(m0, CoLending.computePoolManifest([{ ...base[0], originationDate: "2023-01-01" }]));
});
...
function sealTape(loans: CoLending.TapeInputLoan[]): CoLending.AssurePoolTape {
...
test("H6 — a post-freeze reclassification FAILS verifyTape with the manifest-mismatch reason", () => {
  ...
  const original: CoLending.TapeInputLoan[] = [
    { loanRef: "L1", verdict: "ELIGIBLE", overridden: false, disbursedMinor: "90000", originationDate: "2024-06-15", classificationBucket: "STANDARD" },
    { loanRef: "L2", verdict: "WARNING", overridden: false, disbursedMinor: "50000", originationDate: "2024-07-15", classificationBucket: "SMA-1" },
  ];
  const sealedManifest = CoLending.computePoolManifest(original);
  const drifted = original.map((l) => (l.loanRef === "L1" ? { ...l, classificationBucket: "NPA", verdict: "HARD_EXCLUDE" as const } : l));
  const driftedTape = CoLending.buildAssurePoolTape(
  ...
  assert.equal(`sha256:${CoLending.hashObject(body)}`, tapeHash);
