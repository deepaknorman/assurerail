...
// NOTE: this is @Public and unauthenticated. It therefore uses the index-only countNotesByState()
// (never a full listNotes() hydration of every t1Aggregates blob) AND caches the DB tally for 30s, so a
// burst of scrapes cannot be turned into a DoS amplifier. Process gauges stay live (cheap). See
// docs/design/AssureRail_Capacity_And_Load_Testing.md §10.
