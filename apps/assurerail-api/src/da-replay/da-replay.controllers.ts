...
function context(req: RailRequest): RoomActor {
  ...
  if (!req.user.session?.id)
    throw new ForbiddenException("an active Rail session is required");
  if (!req.user.activeInstitution?.institutionId)
    throw new ForbiddenException(
      "an active institution session context is required"
    );
  return {
...
@Controller("v1/rail/cases/:caseId/da-replay")
export class DaReplayController {
  ...
  @Post("authorisation")
  proposeAuthorisation(
    @Req() req: RailRequest,
    @Param("caseId") caseId: string,
    @Body() body: Parameters<DaReplayService["proposeAuthorisation"]>[2]
  ) {
  ...
  reviewAuthorisation(
    ...
    @Body() body: Parameters<DaReplayService["reviewAuthorisation"]>[3]
  ) {
    return this.replay.reviewAuthorisation(
      context(req),
      caseId,
      authorisationId,
      body
  ...
  createSaga(
    @Req() req: RailRequest,
    @Param("caseId") caseId: string,
    @Body() body: Parameters<DaReplayService["createSaga"]>[2]
  ) {
  ...
  observe(
    ...
    @Body() body: Parameters<DaReplayService["recordObservation"]>[4]
  ) {
    return this.replay.recordObservation(
      context(req),
      caseId,
      sagaId,
      legId,
  ...
  reconcile(
    ...
    @Param("sagaId") sagaId: string,
    @Param("legId") legId: string,
    @Body() body: Parameters<DaReplayService["reconcileLeg"]>[4]
  ) {
  ...
  proposeRepair(
    ...
    @Body() body: Parameters<DaReplayService["proposeRepair"]>[3]
  ) {
  ...
  reviewRepair(
    ...
    @Body() body: Parameters<DaReplayService["reviewRepair"]>[4]
  ) {
    return this.replay.reviewRepair(
      context(req),
      caseId,
      breakId,
      repairId,
  ...
  @Get("sagas/:sagaId/comparison.csv")
  async comparisonCsv(
    @Req() req: RailRequest,
    ...
    response.attachment(`assurerail-da-replay-${sagaId}-comparison.csv`);
    return this.replay.comparisonCsv(context(req), caseId, sagaId);
  }
