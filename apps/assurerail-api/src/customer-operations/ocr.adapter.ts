...
async function modelCall(provider:"openai"|"gemini",model:string,tier:ModelTier,prompt:string,schema:unknown,text:string,document:AiDocument|undefined,http:typeof fetch){
  ...
  let result:any;try{result=JSON.parse(Buffer.concat(chunks).toString("utf8"));}catch{throw new Error("AI_RESPONSE_INVALID_JSON");}let output:string;
  ...
  let parsed:unknown;try{parsed=JSON.parse(output);}catch{throw new Error("AI_RESPONSE_INVALID_JSON");}
  return {provider,model,modelTier:tier,result:parsed,usage:google?result.usageMetadata:result.usage};
}
