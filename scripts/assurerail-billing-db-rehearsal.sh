...
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/assurerail-billing.XXXXXX")"
...
cleanup() {
  ...
  case "$TEST_ROOT" in */assurerail-billing.*) rm -rf -- "$TEST_ROOT" ;; esac
}
...
echo "[BILLING-DB] exact commercial records, maker-checker receipts, service history and exit manifest"
psql_db "$FRESH_DB" -c '
...
case "$schema_diff" in *CustomerContract*|*CustomerRateCard*|*CustomerInvoice*|*CustomerService*|*CustomerDataExit*|*CustomerPaymentReceipt*) echo "$schema_diff" >&2; exit 1 ;; esac
...
echo "[BILLING-DB] PASS contract=1 fee-rules=2 invoice=1 service-request=1 exit=1 restore=1|2|1|1|1 external-customer-evidence=none"
