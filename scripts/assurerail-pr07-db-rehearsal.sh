...
while IFS= read -r migration_file; do psql_db "$UPGRADE_DB" -f "$migration_file" >/dev/null; done < <(find "$MIGRATIONS_DIR" -mindepth 2 -maxdepth 2 -name migration.sql | sort | awk -v target="/$PR07_MIGRATION/" 'index($0,target){exit} {print}')
