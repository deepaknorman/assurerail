...
const API = `${ROOT}/apps/assuredst-api`;
...
const tracked = git("git ls-files apps/assuredst-api").split("\n").filter(Boolean);
if (tracked.some((f) => f.endsWith("/.env") || f === "apps/assuredst-api/.env")) bad(".env is TRACKED by git — must be gitignored (secrets leak)");
else pass(".env is not tracked");
const ignored = git("git check-ignore apps/assuredst-api/.env");
...
const main = rd("apps/assuredst-api/src/main.ts");
...
const dvp = rd("apps/assuredst-api/src/dvp/dvp.service.ts");
const mint = rd("apps/assuredst-api/src/mint/mint.service.ts");
const prismaRepo = rd("apps/assuredst-api/src/store/prisma-mint.repository.ts");
...
const schema = rd("apps/assuredst-api/prisma/schema.prisma");
...
const cfg = rd("apps/assuredst-api/src/config.ts");
