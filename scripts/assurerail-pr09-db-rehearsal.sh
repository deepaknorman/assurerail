...
while IFS= read -r migration_file; do
  ...
  [[ "$migration_name" < "$PR09_MIGRATION" ]] || continue
  psql_db "$UPGRADE_DB" -f "$migration_file" >/dev/null
done < <(find "$MIGRATIONS_DIR" -mindepth 2 -maxdepth 2 -name migration.sql | sort)
